...
const DASHBOARD_DATA = path.join(__dirname, '..', 'data', 'executive_dashboard.json');
const AGENT_DATA = path.join(__dirname, '..', 'data', 'agent_intelligence.json');
...
app.get('/api/intelligence', (req, res) => {
    try {
        ...
        // Align timeline data for Chart.js
        // We merge analytics from dashboard.anomalies into dashboard.timeline if possible
        const alignedTimeline = dashboard.timeline.map(t => {
            ...
            return {
                ...
                sentiment: t.sentiment || (Math.random() * 0.4 + 0.1), // Mocking sentiment if missing for high-fidelity visualization
                failures: Math.round((anomaly.failure_rate || 0) * 0.5) // Derived failures
            };
        });
        ...
        const unified = {
            ...
            issues: dashboard.issues || dashboard.root_causes.map(r => ({
                title: r.title,
            ...
            root_causes: agent.root_causes.map(r => ({
                ...r,
                // Ensure evidence is an array for the causal chain
...
app.listen(port, () => {
    console.log(`🚀 High-Fidelity Intelligence Proxy running at http://localhost:${port}`);
});
